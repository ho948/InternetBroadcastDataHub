...
WITH RankedGameVideos AS (
    SELECT
        ytgr.rk, ytgv.link, ytgv.title, ytgv.views_count, ytgv.uploaded_at, 
        ytgv.thumbnail_img, ytgv.video_text, ytgv.channel_link, ytgv.channel_name, 
        ytgv.subscribers_count, ytgv.channel_img, ytgv.execution_ts,
        ROW_NUMBER() OVER (PARTITION BY ytgv.link, ytgv.execution_ts ORDER BY ytgr.rk) AS row_num
    FROM raw.youtube_trending_game_videos ytgv
    JOIN raw.youtube_trending_game_ranks ytgr 
      ON ytgv.link = ytgr.link AND ytgv.execution_ts = ytgr.execution_ts AND ytgv.views_count > 10000
)
