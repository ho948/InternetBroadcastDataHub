...
WITH ranked_channels AS (
    SELECT 
        date_trunc('hour', l.execution_ts) AS ts,
        l.channel_id,
        c.channel_name,
        MAX(l.viewers_count) AS peak_viewers_count,
        RANK() OVER (PARTITION BY date_trunc('hour', l.execution_ts) ORDER BY MAX(l.viewers_count) DESC) AS rank
    FROM raw.chzzk_popular_lives l
    JOIN raw.chzzk_popular_channels c
    ON l.channel_id = c.channel_id
    GROUP BY date_trunc('hour', l.execution_ts), l.channel_id, c.channel_name
)
...
WITH ranked_channels AS (
    SELECT 
        date_trunc('day', l.execution_ts) AS date,
        l.channel_id,
        c.channel_name,
        MAX(l.viewers_count) AS peak_viewers_count,
        RANK() OVER (PARTITION BY date_trunc('day', l.execution_ts) ORDER BY MAX(l.viewers_count) DESC) AS rank
    FROM raw.chzzk_popular_lives l
    JOIN raw.chzzk_popular_channels c
    ON l.channel_id = c.channel_id
    GROUP BY date_trunc('day', l.execution_ts), l.channel_id, c.channel_name
)
...
SELECT 
    l.live_id,
    l.open_ts,
    (l.execution_ts - l.open_ts) AS time_to_peak
FROM raw.chzzk_popular_lives l
JOIN (
...
SELECT 
    ...
    SUM(l.viewers_count) AS total_viewers_count
FROM raw.chzzk_popular_lives l
GROUP BY l.category_id, DATE_TRUNC('day', l.execution_ts);